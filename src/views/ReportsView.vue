  ...
  <div class="reports-container bg-gray-50 min-h-screen p-6 relative">
    ...
    <div class="max-w-7xl mx-auto mt-8">
      ...
      <div class="content-area-container bg-white p-5 rounded-lg shadow-sm mb-6">
        ...
        <!-- Summary Row -->
        <div class="flex flex-wrap gap-8 mb-6 text-lg font-semibold">
          <div>Total Sales: <span class="text-blue-600">₱{{ totalSales }}</span></div>
          <div>Items Sold: <span class="text-green-600">{{ itemsSold }}</span></div>
          <div>Total Orders: <span class="text-yellow-600">{{ totalOrders }}</span></div>
        </div>
        ...
        <div>
          ...
          <div class="overflow-x-auto">
            <DataTable :value="filteredData" paginator :rows="10" stripedRows style="min-width: 800px;">
...
function exportCSV() {
  ...
  const rows = filteredData.value.map(r => [r.date, r.category, r.user, r.item, r.quantity, r.total])
...
function exportPDF() {
  ...
  autoTable(doc, {
    ...
    body: filteredData.value.map(r => [r.date, r.category, r.user, r.item, r.quantity, r.total])
  })
